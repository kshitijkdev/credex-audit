...
// Use service role key — this route needs to read all rows
...
export async function POST(req: NextRequest) {
  // Protect with a secret so only you/cron can trigger it
  const body = await req.json().catch(() => ({}));
  if (body.secret !== process.env.DETECT_CHANGES_SECRET) {
    return NextResponse.json({ error: "unauthorized" }, { status: 401 });
  }

  // 1. Fetch all non-stale audits that have an email + pricing snapshot
  ...
  // Group affected audits by user email for consolidated emails
  const byUser: Record<
    string,
    {
      email: string;
  ...
  for (const audit of audits ?? []) {
    ...
    // Re-run with current pricing to see if recommendations changed
    ...
    // Only flag if recommendations actually differ
...
// Also support GET for easy manual browser trigger during review
...
async function sendStaleEmail(userData: {
  ...
  // Deduplicate changes across audits
  const allChanges = [
    ...new Set(userData.audits.flatMap((a) => a.changes)),
  ];
