...
export default async function ReauditPage({
  ...
  // Reconstruct old result from stored data
  ...
  const savingsDelta =
    newResult.totalMonthlySavings - (oldResult?.totalMonthlySavings ?? 0);

  // Find tools where recommendation changed
  const changedTools = newResult.results.filter((newTool) => {
    const oldTool = oldResult?.results?.find(
      (o: ToolAuditResult) => o.tool === newTool.tool
    );
    if (!oldTool) return false;
    return (
      oldTool.flag !== newTool.flag ||
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-slate-400">
            Pricing has changed since your original audit. Here&apos;s what
            that means for your recommendations.
          </p>
        </div>

        {/* Savings delta banner */}
        <div
            ...
            <div>
              ...
              <div className="flex items-baseline gap-3">
                <span className="text-slate-500 line-through text-xl">
                  ${oldResult?.totalMonthlySavings?.toFixed(0) ?? "?"}/mo
                </span>
                <span className="text-3xl font-bold text-white">
                  ${newResult.totalMonthlySavings.toFixed(0)}/mo
                </span>
                {savingsDelta !== 0 && (
                  <span
        ...
        {/* Changed tools callout */}
        {changedTools.length > 0 && (
          <div className="bg-amber-900/20 border border-amber-800 rounded-xl p-4 mb-6">
            <p className="text-amber-300 font-medium text-sm">
              {changedTools.length} recommendation
              {changedTools.length > 1 ? "s" : ""} changed:{" "}
              {changedTools.map((t) => t.toolLabel).join(", ")}
            </p>
          </div>
        )}
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <h3 className="font-semibold text-white text-lg">
                    {newTool.toolLabel}
                    {changed && (
                      <span className="ml-2 text-xs font-normal text-amber-400 bg-amber-900/30 px-2 py-0.5 rounded-full">
                        Changed
                      </span>
                    )}
                  </h3>
                ...
                {changed && oldTool ? (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      <p className="text-sm text-slate-300">
                        {oldTool.recommendedAction}
                      </p>
                      {oldTool.estimatedSavings > 0 && (
                        <p className="text-xs text-slate-500 mt-1">
                          Savings: ${oldTool.estimatedSavings}/mo
                        </p>
                      )}
                    </div>
                    {/* New recommendation */}
                    <div
                      className={`rounded-lg border p-3 ${FLAG_BG[newTool.flag]}`}
                    >
                      <p className="text-xs text-slate-400 uppercase tracking-wide mb-1">
                        Updated
                      </p>
                      <p className="text-sm text-slate-100">
                        {newTool.recommendedAction}
                      </p>
                      {newTool.estimatedSavings > 0 && (
                        <p className="text-xs text-slate-400 mt-1">
                          Savings: ${newTool.estimatedSavings}/mo
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  >
                    ...
                    {newTool.estimatedSavings > 0 && (
                      <p className="text-xs text-slate-400 mt-1">
                        Estimated savings: ${newTool.estimatedSavings}/mo
                      </p>
                    )}
                  </div>
                )}
        ...
        <div className="mt-10 text-center">
          ...
          >
            Run a fresh audit with current pricing
          </a>
        </div>
      </div>
    </main>
